...
async function scrapeLL() {
  ...
  if (pitTbl) {
    pitTbl.slice(1).forEach(r => {
      ...
      const t = teams.find(t => t.team && name.toLowerCase().includes(t.team.toLowerCase().split(" ")[0].toLowerCase()));
  ...
  const pitchers = indPitTbl ? indPitTbl.slice(1).filter(r => r[1] && r[1].includes("(")).map(r => {
    const p = r[1].split("(");
    return { name: p[0].trim(), team: p[1] ? p[1].replace(")","").trim() : "",
